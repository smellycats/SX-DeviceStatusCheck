...
class WatchDog(object):
    ...
    def mul_ping(self, dev_list):
        ...
        for i, j in zip(dev_list, rel):
            if j is True:
                dev_true_list.append({'ip': i['ip'], 'status': True})
                continue
            if j is False and i['status'] is False:
                dev_true_list.append({'ip': i['ip'], 'status': False})
                continue
            dev_false_list.append({'ip': i['ip'], 'status': False})
    ...
    def device_status_check(self):
        ...
        dev_true_list = []
        dev_false_list = self.dev.get_device_check(num=24, type=n)['items']
